 ...
 */
document
  .getElementById("incremento-rollo-compra")
  .addEventListener("input", function () {
    document.getElementById("val-incremento-rollo-compra").textContent =
      this.value;
...
document
  .getElementById("incremento-metro-compra")
  .addEventListener("input", function () {
    document.getElementById("val-incremento-metro-compra").textContent =
      this.value;
    document
      .getElementById("precioTelaMetroReal")
      .dispatchEvent(new Event("input"));
  });
...
document
  ...
  .addEventListener("input", function () {
    ...
    precioRollo = document.getElementById("precioTelaRollo");
    if (
      metros.value != "" &&
      parseInt(metros.value) > 0 &&
      precioRollo.value != "" &&
      parseFloat(precioRollo.value) > 0
    ) {
      this.value = (
        parseFloat(precioRollo.value) / parseFloat(metros.value)
      ).toFixed(1);
      document.getElementById("precioTelaMetro").value = (
        parseFloat(this.value) +
        (parseFloat(this.value) *
          parseInt(document.getElementById("incremento-metro-compra").value)) /
          100
      ).toFixed(1);
    }
  });
  ...
  .addEventListener("input", function () {
    ...
    if (parseInt(this.value) < 1 || !VAL.isNumber(this.value)) {
      ...
      document.getElementById("precioTelaRolloVenta").value = (
        ...
        (parseFloat(this.value) *
          parseFloat(
            document.getElementById("incremento-rollo-compra").value
          )) /
          100
      ).toFixed(1);
    }
...
document
  ...
  .addEventListener("input", function () {
    document.getElementById("val-incremento-metro-compra").textContent = "---";
    document.getElementById("incremento-metro-compra").value = 0;
...
document
  ...
  .addEventListener("input", function () {
    document.getElementById("val-incremento-rollo-compra").textContent = "---";
    document.getElementById("incremento-rollo-compra").value = 0;

    err = document.getElementById("err_precioTelaRolloVenta");
    if (
      parseInt(this.value) <
        parseFloat(document.getElementById("precioTelaRollo").value) ||
...
function validarTabla() {
  ...
  if (tabla.rows.length == 0) {
    document.getElementById("msg-data-incomplete").classList.remove("d-none");
    return 0;
  }
  for (i = 0; i < tabla.rows.length; i++) {
    for (j = 7; j <= 11; j++) {
      if (parseFloat(tabla.rows[i].cells[j].textContent.trim()) <= 0) {
        ...
        document.getElementById("msg-data-incomplete").classList.add("d-none");
      }
    }
  }
