    ...
    fetch("/ImportadoraFernandez/Venta/getTela", {
        ...
        .then((data) => {
            ...
            if (data.status === "success") {
                ...
                metrajeTela.value =
                    parseFloat(stockMetros) * parseInt(stockRollos);

                //metrajeTela.value = 9;
                if (parseInt(metrajeTela.value) < 10) {
                    document
                        .getElementById("msg-alert")
...
function validateTwoInputs() {
    if (stockMetros == null) stockMetros = 1;
    if (stockRollos == null) stockRollos = 1;
    total =
        (stockMetros == 0 ? 1 : stockMetros) *
        (stockRollos == 0 ? 1 : stockRollos);
    ...
    fetch("/ImportadoraFernandez/Sucursal/getSucursal", {
        ...
        .then((data) => {
            if (data.status === "success") {
                ...
                document.getElementById("direcSu").value =
                    data.data["DIRECCION"];
                document.getElementById("telefSu").value =
                    data.data["TELEFONO"];
            } else {
                console.log("ERROR");
            }
